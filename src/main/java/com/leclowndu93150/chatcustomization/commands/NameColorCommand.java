...
public class NameColorCommand extends AbstractPlayerCommand {
    ...
    public NameColorCommand(@Nonnull ChatManager chatManager) {
        super("namecolor", "Set the color of your name in chat");
    ...
    protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store,
        ...
        if (color.isEmpty() || color.equalsIgnoreCase("clear")) {
            chatManager.setColor(playerRef.getUuid(), "name", null);
            context.sendMessage(Message.raw("Your name color has been reset to default.").color("GREEN"));
            return;
        }
        ...
        chatManager.setColor(playerRef.getUuid(), "name", color.toUpperCase());
        String hex = ColorUtil.toHex(color);
        context.sendMessage(Message.raw("Your name color has been set to: ").color("GREEN").insert(Message.raw(color.toUpperCase()).color(hex)));
    }
}
