...
public class MsgColorCommand extends AbstractPlayerCommand {
    ...
    public MsgColorCommand(@Nonnull ChatManager chatManager, @Nonnull Supplier<ChatCustomizationConfig> configSupplier) {
    ...
    protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store,
        ...
        chatManager.setColor(playerRef.getUuid(), "message", color.toUpperCase());
        String hex = ColorUtil.toHex(color);
        context.sendMessage(Message.raw("Your message color has been set to: ").color("GREEN").insert(Message.raw(color.toUpperCase()).color(hex)));
    }
}
